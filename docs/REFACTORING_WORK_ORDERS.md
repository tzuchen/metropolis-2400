...
> **目標**：將專案中的兩大巨型「上帝模組」（`src/renderer.ts` 3,400 行、`src/game.ts` 3,370 行）依職責進行 7 階段模組化拆分，將單檔規模壓縮至 **500 行以內的最佳維護與 AI Review 視窗**，並藉由全套 37 組整合測試確保 **100% 向下相容、零功能破壞**。
...
*驗收指令*：
```bash
npm run typecheck && npx tsx scripts/verify-defeat-cutscene.ts && npm test
```
...
*驗收指令*：
```bash
npm run typecheck && npx tsx scripts/verify-detention-terminal-and-effective-commands.ts && npm test
```
...
*驗收指令*：
```bash
npm run typecheck && npx tsx scripts/verify-renderer.ts && npm test
```
...
*驗收指令*：
```bash
npm run typecheck && npx tsx scripts/verify-plasma-canister-fx-and-npc-intel.ts && npm test
```
...
4. **嚴禁 CommonJS**：專案為 ESM (`"type": "module"`)，新模組一律採用 `import` / `export`，嚴禁使用 `require`。
