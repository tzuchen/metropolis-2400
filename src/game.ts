...
import { getLandmarkKey as getLandmarkKeyFn, checkEnvironmentalLandmarks, checkAdjacentPassageHints as checkAdjacentPassageHintsFn } from './landmarkSystem';
...
export class GameEngine {
  ...
  private getLandmarkKey(): string {
    return getLandmarkKeyFn(this.map?.id || '', this.player.x, this.player.y);
  }
  ...
  private checkEnvironmentalObservations(): void {
    ...
    const result = checkEnvironmentalLandmarks(sectorId, px, py, isZh, this.isConveyorTile(px, py));
    if (result) {
      const key = result.key === 'conveyor' ? `conveyor-${px},${py}` : result.key;
      if (this.lastLandmarkKey !== key) {
        this.lastLandmarkKey = key;
        if (result.floatingText) {
          this.pushFloatingText(px, py, result.floatingText.text, result.floatingText.color);
        }
        if (result.message) {
          this.pushMessage(result.message, 'info');
        }
      }
    }
  ...
  private checkAdjacentPassageHints(): void {
    const result = checkAdjacentPassageHintsFn(
      this.map,
      this.player.x,
      this.player.y,
    ...
    if (result) {
      if (this.lastAdjacentPassageKey !== result.playerKey) {
        this.lastAdjacentPassageKey = result.playerKey;
        this.pushMessage(result.message, 'info');
      }
    } else {
