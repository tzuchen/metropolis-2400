...
export class GameRenderer {
  ...
  drawHud(
    ...
    const consumableItems = [
      { key: '[1]', label: isZh ? `醫療包: ${medCount}` : `MED: ${medCount}`, color: '#00ff88' },
      { key: '[2]', label: isZh ? `電池: ${batCount}` : `BAT: ${batCount}`, color: '#00e5ff' },
      { key: '[3]', label: isZh ? `EMP: ${empCount}` : `EMP: ${empCount}`, color: '#c77dff' },
      { key: '[I]', label: isZh ? '背包' : 'INV', color: '#ffaa00' },
      { key: '[U]', label: isZh ? '黑市' : 'SHOP', color: '#c77dff' },
      { key: '[M]', label: isZh ? '任務' : 'MISSIONS', color: '#00ffaa' },
      { key: '[L]', label: isZh ? '檔案' : 'ARCHIVE', color: '#ffb700' },
      { key: '[P]', label: isZh ? '日記' : 'DIARY', color: '#00f0ff' },
      { key: '[8]', label: isZh ? '存檔' : 'SAVE', color: '#00f0ff' },
      { key: '[9]', label: isZh ? '讀檔' : 'LOAD', color: '#b388ff' },
      { key: '[Z]', label: isZh ? '中' : 'EN', color: '#ffea00' },
      { key: '[B]', label: isZh ? '音樂' : 'BGM', color: '#00ffaa' },
      { key: '[V]', label: isZh ? '全視' : 'OMNI', color: this.isOmniVisionActive ? '#00ffff' : '#667788' },
      { key: '[X]', label: isZh ? '地圖' : 'MAP', color: this.isFullMapActive ? '#ffea00' : '#667788' },
      { key: '[0]', label: isZh ? '解析' : 'RES', color: '#00f0ff' },
      { key: '[TAB]', label: isZh ? '地圖' : 'MAP', color: '#00ffcc' },
      { key: '[F]', label: isZh ? '拔槍' : 'DRAW', color: '#ff3855' },
      { key: '[Q]', label: nextWName ? (isZh ? '換:' + nextWName : 'SWAP:' + nextWName) : (isZh ? '換槍' : 'SWAP'), color: '#b388ff' },
    ];
